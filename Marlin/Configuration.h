...
#define STRING_CONFIG_H_AUTHOR                                                 \
  "(Bob Kuhn, Anet config)" // Who made the changes.
...
#define CUSOM_MACHINE_NAME "Hydrogen"
 ...
 * 'TMC2208_STANDALONE', 'TMC2209', 'TMC2209_STANDALONE', 'TMC2660',
...
// Volumetric, Filament Width Sensor, etc. The Anet A8 original extruder is
// designed for 1.75mm
 ...
 * nozzles. Can be combined with SWITCHING_EXTRUDER.
 */
...
// #define PSU_DEFAULT_OFF             // Keep power off until enabled directly
// with M80 #define PSU_POWERUP_DELAY      250  // (ms) Delay for the PSU to
// warm up to full power #define LED_POWEROFF_TIMEOUT 10000  // (ms) Turn off
// LEDs after power-off, with this amount of delay

// #define PSU_OFF_REDUNDANT           // Second pin for redundant power control
// #define PSU_OFF_REDUNDANT_INVERTED  // Redundant pin state is the inverse of
// PSU_ACTIVE_STATE

// #define PS_ON1_PIN               6  // Redundant pin required to enable power
// in combination with PS_ON_PIN

// #define PS_ON_EDM_PIN            8  // External Device Monitoring pins for
// external power control relay feedback. Fault on mismatch. #define
// PS_ON1_EDM_PIN           9
 ...
 * SMD NTCS0603E3104FXT (#8) - calibrated for Makibox hot bed 13 : 100kΩ Hisens
...
#define TEMP_RESIDENCY_TIME                                                    \
  6 // (seconds) Time to wait for hotend to "settle" in M109
#define TEMP_WINDOW                                                            \
...
#define TEMP_BED_RESIDENCY_TIME                                                \
  6 // (seconds) Time to wait for bed to "settle" in M190
...
// "Advanced Settings" menu. (~1.3K bytes of flash) #define MPC_AUTOTUNE_MENU //
...
// for 8 cycles.
#else
// #define BED_LIMIT_SWITCHING   // Keep the bed temperature within
// BED_HYSTERESIS of the target
#endif
...
#define PID_FUNCTIONAL_RANGE                                                   \
  15 // If the temperature difference between the target temperature and the
...
#define EXTRUDE_MINTEMP                                                        \
  160 // 160 guards against false tripping when the extruder fan kicks on.
...
// #define MARKFORGED_INVERSE  // Enable for an inverted Markforged kinematics
// belt path
#endif
 ...
 *
 * 3D printing:
 * Movements very close to the center of the polar axis take more time than
 * others. This brief delay results in more material deposition due to the
 * pressure in the nozzle.
 *
 * Current Kinematics and feedrate scaling deals with this by making the
 * movement as fast as possible. It works for slow movements but doesn't work
 * well with fast ones. A more complicated extrusion compensation must be
 * implemented.
 *
 * Ideally, it should estimate that a long rotation near the center is ahead and
 * will cause unwanted deposition. Therefore it can compensate the extrusion
 * beforehand.
 *
 * Laser cutting:
 * Same thing would be a problem for laser engraving too. As it spends time
 * rotating at the center point, more likely it will burn more material than it
 * should. Therefore similar compensation would be implemented for laser-cutting
 * operations.
 *
 * Milling:
...
// Movements fall inside POLAR_FAST_RADIUS are assigned the highest possible
// feedrate to compensate unwanted deposition related to the near-origin motion
// problem.
...
#define DEFAULT_ACCELERATION                                                   \
  400 // X, Y, Z ... and E acceleration for printing moves
...
#define DEFAULT_ANGULAR_TRAVEL_ACCELERATION                                    \
  3000 // I, J, K acceleration for rotational-only travel (non printing) moves
#endif
...
// #define Z_SERVO_MEASURE_ANGLE 45      // Use if the servo must move to a
// "free" position for measuring after deploy #define Z_SERVO_INTERMEDIATE_STOW
// // Stow the probe between points #define Z_SERVO_DEACTIVATE_AFTER_STOW //
// Deactivate the servo when probe is stowed
#endif
...
// #define BD_SENSOR_PROBE_NO_STOP // Probe bed without stopping at each probe
// point
#endif
...
// https://docs.duet3d.com/en/Duet3D_hardware/Accessories/Smart_Effector When
// the pin is defined you can use M672 to set/reset the probe sensitivity.
 ...
 * https://reprap.org/wiki/Kossel#Autolevel_probe Deploys by touching z-axis
 * belt. Retracts by pushing the probe down.
 */
...
// #define Z_CLEARANCE_FOR_HOMING  4   // (mm) Minimal Z height before homing
// (G28) for Z clearance above the bed, clamps, ...
//  You'll need this much clearance above Z_MAX_POS to avoid grinding.

// #define Z_AFTER_HOMING         10   // (mm) Height to move to after homing
// (if Z was homed) #define XY_AFTER_HOMING { 10, 10 }  // (mm) Move to an XY
// position after homing (and raising Z)

// #define EVENT_GCODE_AFTER_HOMING "M300 P440 S200"  // Commands to run after
// G28 (and move to XY_AFTER_HOMING)
...
// #define UBL_TILT_ON_MESH_POINTS         // Use nearest mesh points with G29 J
// for better Z reference #define UBL_TILT_ON_MESH_POINTS_3POINT  // Use nearest
// mesh points with G29 J0 (3-point)
...
#define OBSTACLE1                                                              \
  {                                                                            \
    (X_BED_SIZE) / 4 - (CLIP_W) / 2, 0, (X_BED_SIZE) / 4 + (CLIP_W) / 2,       \
        CLIP_H                                                                 \
  }
#define OBSTACLE2                                                              \
  {                                                                            \
...
#define BED_TRAMMING_INCLUDE_CENTER // Move to the center after the
// last corner #define BED_TRAMMING_USE_PROBE
 ...
 */
#define BED_TRAMMING_LEVELING_ORDER { LF, RB, LB, RF } // { LF, RF, RB, LB }
// 
#endif
...
// #define Z_SAFE_HOMING_POINT_ABSOLUTE  // Ignore home offsets (M206) for Z
// homing position
#endif
...
// like keepalive messages #define DEFAULT_KEEPALIVE_INTERVAL 2 // Number of
// seconds between "busy" messages. Set with M113.
...
// Default pattern to use when 'P' is not provided to G12. One of the enabled
// options above.
...
// #define PASSWORD_ON_SD_PRINT_MENU       // This does not prevent G-codes from
...
//
// #define STARTUP_TUNE { 698, 300, 0, 50, 523, 50, 0, 25, 494, 50, 0, 25, 523,
// 100, 0, 50, 554, 300, 0, 100, 523, 300 }
...
#define ZONESTAR_LCD // Requires ADC_KEYPAD_PIN to be assigned to an analog pin.
                     // This LCD is known to be susceptible to electrical
                     // interference which scrambles the display.  Pressing any
                     // button clears it up. This is a LCD2004 display with 5
                     // analog buttons.
...
//
// Note: Details on connecting to the Anet V1.0 controller are in the file
// pins_ANET_10.h
//
...
//
// BigTreeTech Mini 12864 V1.0 / V2.0 is an alias for FYSETC_MINI_12864_2_1.
// Type A/B. NeoPixel RGB Backlight. https://github.com/bigtreetech/MINI-12864
//
// #define BTT_MINI_12864

//
// BEEZ MINI 12864 is an alias for FYSETC_MINI_12864_2_1. Type A/B. NeoPixel RGB
// Backlight.
//
 ...
 * IA_CREALITY (T5UID1)
 *  - Download
 * https://github.com/InsanityAutomation/Marlin/raw/CrealityDwin_2.0/TM3D_Combined480272_Landscape_V7.7z
 *  - Copy the downloaded DWIN_SET folder to the SD card.
 *
 * E3S1PRO (T5L)
 *  - Download
 * https://github.com/CrealityOfficial/Ender-3S1/archive/3S1_Plus_Screen.zip
 *  - Copy the downloaded DWIN_SET folder to the SD card.
 *
 * CREALITY_TOUCH
...
/**
 * TFT Theme for Color_UI. Choose one of the following or add a new one to
...
// #define TFT_SHARED_IO   // I/O is shared between TFT display and other
